const getStartedForm=document.querySelector("#getStartedForm"),userName=document.querySelector("#name"),ctaWrapper=document.querySelector(".cta-wrapper"),taskAdderForm=document.querySelector("#taskAdderForm"),taskDescription=document.querySelector("#taskDescription"),tasksContainer=document.querySelector("#tasks-list .container"),welcomeContent=document.querySelector("#main"),taskApp=document.querySelector("#plan-app"),hideForLoader=document.querySelector("#hide-for-loader"),loader=document.querySelector(".loader");let tasksCounter=0;const noTasks=document.createElement("p");noTasks.className="center",noTasks.textContent="You don't have any tasks to complete, yet.",loadEventListeners();function loadEventListeners(){document.addEventListener("DOMContentLoaded",appInit),getStartedForm.addEventListener("submit",showTaskAdder),tasksContainer.addEventListener("click",removeTask)}function appInit(){hideForLoader.style.display="none",loader.style.display="block",setTimeout(function(){hideForLoader.style.display="block",loader.style.display="none",taskApp.classList.add("hide");const a=loadUserTasksFromLS();a.length?(a.forEach(a=>{const b=document.createElement("li");b.className="task-list-item",tasksCounter++,b.innerHTML=`
                    ...
                    `,tasksContainer.appendChild(b)}),welcomeContent.style.display="none",taskApp.style.display="block",setTimeout(function(){taskApp.classList.remove("hide"),taskApp.classList.add("show")},1e3),taskAdderForm.addEventListener("submit",addTask)):tasksContainer.appendChild(noTasks)},1e3)}function showTaskAdder(a){if(a.preventDefault(),""!==userName.value)setTimeout(function(){welcomeContent.style.display="none",loader.style.display="block",taskApp.style.display="block",setTimeout(function(){taskApp.classList.remove("hide"),taskApp.classList.add("show"),loader.style.display="none"},1e3)},1e3),taskAdderForm.addEventListener("submit",addTask);else{const a=document.createElement("span");a.className="error-text",a.textContent="* Please enter your name first.",ctaWrapper.insertBefore(a,ctaWrapper.firstChild)}}function addTask(a){if(a.preventDefault(),""===taskDescription.value)taskDescription.classList.add("error");else{const a=document.createElement("li");a.className="task-list-item",tasksCounter++,a.innerHTML=`
            <input type="checkbox" id="task-${tasksCounter}">
